...
function PocketMeroe_MenuToggle()
	local menu = PocketMeroeOptionsWindow or PocketMeroe_CreateMenu();
	if (menu) then
...
function PocketMeroe_CreateMenu()
	...
	if (PocketMeroeOptionsWindow) then
		PocketMeroeOptionsWindow:Show()
		return
	end

	--build the options window
	local PocketMeroeOptionsWindow = DF:CreateSimplePanel (UIParent, 560, 330, "pocketMeroe Config", "pocketMeroeOptionsFrame")
	--local PocketMeroeOptionsWindow = DF:NewPanel(UIParent, _, "PocketMeroeOptionsWindow", _, 897, 592)
	local optionsFrame = PocketMeroeOptionsWindow
	...
	local authorInfo = DF:CreateLabel(statusBar, "For |cFFFFFFFF<Serenity>|r - Mankrik, by |cFFFFFFFFmeroe|r")
	...
	statusBar:SetFrameLevel(PocketMeroeOptionsWindow:GetFrameLevel()+2)
	...
	local bottomGradient = DF:CreateTexture(PocketMeroeOptionsWindow, {gradient = "vertical", fromColor = {0, 0, 0, 0.3}, toColor = "transparent"}, 1, 100, "artwork", {0, 1, 0, 1}, "bottomGradient")
	bottomGradient:SetPoint("bottom-top", statusBar)

	--divisor shown above the tab options area
	local frameBackgroundTextureTopLine = PocketMeroeOptionsWindow:CreateTexture("$parentHeaderDivisorTopLine", "artwork")
	...
	local frameBackgroundTextureLeftLine = PocketMeroeOptionsWindow:CreateTexture("$parentHeaderDivisorLeftLine", "artwork")
	local divisorYPosition = -60
	...
	local tabList = {
		...
		{name = "castersConfig",	text = "Casters"},
	...
	local tabContainer = DF:CreateTabContainer(PocketMeroeOptionsWindow, "pocketMeroe", PocketMeroeOptionsWindow, tabList, optionsTable, hookList)
												
	tabContainer:SetPoint("center", optionsFrame, "center", 0, 0)
	tabContainer:SetSize(PocketMeroeOptionsWindow:GetSize())
	tabContainer:Show()
	...
	for index, frame in ipairs(tabContainer.AllFrames) do
		...
		frameBackgroundTexture:SetPoint("topleft", frame, "topleft", 4, -80)
	...
	do
		...
		DF:BuildMenu(generalSettingsFrame, optionsTable, 10, -88, tabFrameHeight, false, options_text_template, options_dropdown_template, options_switch_template, true, options_slider_template, options_button_template, profileCallback)
	...
	do
		...
		DF:BuildMenu(marksConfigFrame, optionsTable, 10, -88, 340, false, options_text_template, options_dropdown_template, options_switch_template, true, options_slider_template, options_button_template, profileCallback)

		local skull = marksConfigFrame:CreateTexture(nil, "overlay")
		skull:SetTexture([[Interface\TargetingFrame\UI-RaidTargetingIcon_8]])
		skull:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget2, "left", -8, 0)
		...
		cross:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget3, "left", -8, 0)
		...
		square:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget4, "left", -8, 0)
		...
		moon:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget5, "left", -8, 0)
		...
		triangle:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget6, "left", -8, 0)
		...
		diamond:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget7, "left", -8, 0)
		...
		circle:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget8, "left", -8, 0)
		...
		star:SetPoint("right", pocketMeroeOptionsFramemarksConfigWidget9, "left", -8, 0)
	...
	do
		...
		DF:BuildMenu(markingConfigFrame, optionsTable, 10, -88, tabFrameHeight, false, options_text_template, options_dropdown_template, options_switch_template, true, options_slider_template, options_button_template, profileCallback)
		...
		if (not pocketMeroeScrollPanel) then
			...
			markingScroll:SetPoint("TOPLEFT", markingConfigFrame, "TOPLEFT", 5, -130)
...
--[[ 	local scenario = CreateFrame("frame", "pocketMeroeScenario", UIParent, "BackdropTemplate")
	...
	PocketMeroeOptionsWindow:Hide();
	return PocketMeroeOptionsWindow;
end
------ Auto-Marking ----------------------------------------------------------------------------------------------------
